...
#define	CONFIG_SERVERIP					192.168.1.162
...
#define CONFIG_BOOT_NFS \
	"fatload mmc ${mmcdev}:${mmcpart} ${loadaddr} zImage;" \
	"fatload mmc ${mmcdev}:${mmcpart} ${fdt_addr} imx6q-sabresd.dtb;" \
	"run netargs;" \
...
#define CONFIG_BOOTANDROID \
	"run mmcargs;"	\
	"fatload mmc ${mmcdev}:${mmcpart} ${loadaddr} boot.img;"	\
	"boota ${loadaddr}"
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"dh=dhcp\0"	\
	"logoimgname=logo.bmp\0"	\
	"nfs_root=setenv mmcroot /dev/nfs rootwait rw nfsroot=${nfsserverip}:${nfsroot},v3,tcp\0"	\
	"nfsroot=/nfs/imx6_corec\0"	\
	"update_sd_firmware=" \
		"if test ${ip_dyn} = yes; then " \
			"setenv get_cmd dhcp; " \
		"else " \
	...
	"tftp_loadimage=tftpfat ${tftpimage} mmc ${mmcdev}:${mmcpart} ${image}\0"	\
	"tftp_loadfdt=tftpfat ${fdt_file} mmc ${mmcdev}:${mmcpart} imx6q-sabresd.dtb\0"	\
	"tftp_loadlogo=tftpfat ${logoimgname} mmc ${mmcdev}:${mmcpart} logo.bmp\0"	\
	"mmcargs=setenv bootargs console=${console},${baudrate} init=/init ${smp} " \
	"root=${mmcroot} ${mxcfb0} ${mxcfb1} ${mxcfb2} ${mxcfb3} vmalloc=256M androidboot.console=ttySAC0 consoleblank=0 androidboot.hardware=freescale cma=384M\0" \
	...
	"netargs=setenv bootargs console=${console},${baudrate} ${smp} " \
		"ip=${dh} init=/init root=/dev/nfs rootwait rw " \
		"nfsroot=${nfsserverip}:${nfsroot},v3,tcp " \
		"${mxcfb0} ${mxcfb1} ${mxcfb2} ${mxcfb3} vmalloc=400M androidboot.console=ttySAC0 androidboot.hardware=freescale cma=384M\0"	\
	"netboot=echo Booting from net ...; " \
		"run netargs; " \
		"if tftp ${tftpimage}; then " \
...
#define CONFIG_BOOTCOMMAND \
	"run mmcargs;"	\
	"fatload mmc 2:1 0x12000000 boot.img;"	\
	"boota 0x12000000"
*/
